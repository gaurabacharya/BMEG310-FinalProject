...
4. Analysis
```{r}
# Align clinical data with patient clusters
aligned_clinical_data <- clinical_data %>%
  filter(PATIENT_ID %in% patient_cluster_assignments$Patient)

# Merge clinical data with cluster assignments
clinical_with_clusters <- aligned_clinical_data %>%
  left_join(patient_cluster_assignments, by = c("PATIENT_ID" = "Patient"))


# Summary statistics for age by cluster
summary_by_age <- clinical_with_clusters %>%
...
# Count distribution of categorical variables (e.g., sex, race)
sex_distribution <- clinical_with_clusters %>%
  group_by(Ward_Cluster, SEX) %>%
  summarise(Count = n())

race_distribution <- clinical_with_clusters %>%
  group_by(Ward_Cluster, RACE) %>%
  summarise(Count = n())
...
Age vs Patient Cluster
```{r}
# Fix: Ensure Ward_Cluster is a factor
clinical_with_clusters$Ward_Cluster <- as.factor(clinical_with_clusters$Ward_Cluster)

# Create the box plot
ggplot(clinical_with_clusters, aes(x = Ward_Cluster, y = as.numeric(AGE), fill = Ward_Cluster)) +
  geom_boxplot() +
  labs(title = "Age Distribution by Patient Cluster", x = "Cluster", y = "Age") +
  theme_minimal()

```

Sex vs Patient Cluster
```{r}
# Fix: Group by Ward_Cluster
ggplot(sex_distribution, aes(x = Ward_Cluster, y = Count, fill = SEX, group = SEX)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Sex Distribution by Patient Cluster", x = "Cluster", y = "Count") +
  theme_minimal()

```

Race vs Patient Cluster
```{r}

clinical_with_clusters$RACE[clinical_with_clusters$RACE == ""] <- "Unknown"
clinical_with_clusters$RACE[is.na(clinical_with_clusters$RACE)] <- "Unknown"

race_distribution <- clinical_with_clusters %>%
  group_by(Ward_Cluster, RACE) %>%
  summarise(Count = n(), .groups = "drop")

ggplot(race_distribution, aes(x = Ward_Cluster, y = Count, fill = RACE, group = RACE)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Race Distribution by Patient Cluster", x = "Cluster", y = "Count") +
  theme_minimal()


# Check total counts by cluster
race_distribution <- clinical_with_clusters %>%
  group_by(Ward_Cluster, RACE) %>%
  summarise(Count = n(), .groups = "drop")
...
# Check how many "Unknown" were added
table(clinical_with_clusters$RACE)
